class LCA_EIO_BoxPlot {
    constructor(parentElement, Stage, yLimLow, yLimHigh, Data, TallyData,TallyData_noBio) {
    ...
    initVis() {
        ...
        vis.wrangleData()
        d3.select('#saveButton_LCA_D').on('click', function(){
    ...
    wrangleData() {
        ...
        vis.filteredData = vis.Data.filter(function (d) {
            return (d.Stages === vis.Stage)
        })
        //console.log(vis.TallyData)
        vis.filteredDataTally = vis.TallyData.filter(function (d) {
            return (d.Stages === vis.Stage)
        })
        vis.filteredDataTally_noBio = vis.TallyData_noBio.filter(function (d) {
            return (d.Stages === vis.Stage)
        })
        vis.filteredDataG = Array.from(d3.group(vis.filteredData, d => d.Material), ([key, value]) => ({key, value}))
        vis.filteredTallyG = Array.from(d3.group(vis.filteredDataTally, d => d.Material), ([key, value]) => ({
            key,
            value
        }))
        vis.filteredTally_noBioG = Array.from(d3.group(vis.filteredDataTally_noBio, d => d.Material), ([key, value]) => ({
            key,
            value
        }))
